...
**자막의 1번 규칙: 편집본을 재전사하지 않는다.** `list_sequence_tracks(includeSourceTimes:true)`가 반환하는 클립별 소스 in/out이 곧 source→timeline 맵이다. 원본을 한 번만 전사하고 그 워드 타임스탬프를 재투영하면, 몇 번을 재편집해도 프레임 단위로 맞는다. 재전사는 느리고 누적 교정을 날리며 품질도 떨어진다(실측: `수정 버튼`→`주종범`).
