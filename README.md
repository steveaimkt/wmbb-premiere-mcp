...
①~③ 까지 하면 AI 클라이언트가 프리미어를 조작할 수 있다. 하지만 서버가 주는 것은 **툴 26개**뿐이다. 어떤 순서로 분석하고, 어디서 멈춰 승인을 받고, 무엇을 기준으로 자를지는 정해져 있지 않다.

**스킬은 그 작업 절차를 적은 파일이다.** 실제 촬영본을 편집하면서 정한 기준값(몇 초 이상의 공백을 자를지, 인터뷰에서는 반복 탐지를 믿지 않는다 등)과, 한 번 실수해서 재편집한 뒤 추가한 확인 단계가 들어 있다. 스킬 없이도 툴은 동작하지만, 그러면 매번 순서와 기준을 말로 설명해야 한다.

#### 설치되는 스킬 3개

| 스킬 | 부르는 말 | 하는 일 | 결과물 |
|---|---|---|---|
| `프리미어-컷편집` | 「컷편집 시작하자」 | 대상 확인 → 음원 분석 → 본편 시작점 찾기 → 컷 구간 카테고리별 제안 → 화면·파형 확인 → **사용자 승인** → 삭제 → 실측 검증 | 편집된 타임라인. 저장은 사용자가 「저장」이라고 할 때만 한다 |
| `자막-검수` | 「자막 검수 시작하자」 | 타임라인 매핑 → 원본 전사를 편집본에 맞춰 옮김 → 한 줄 20자로 큐 나누기 → 오탈자 교정 → 품질 검사 | 프로젝트(`.prproj`)와 같은 폴더에 `<프로젝트명>_최종_<길이>.srt` |
| `프리미어-MCP-설치` | 「프리미어 MCP 설치 점검」 | 이 설치 과정을 다시 점검한다 | 점검 결과 |

컷편집 스킬은 단계마다 멈추고 확인을 받는다. 승인 없이 삭제까지 가지 않는다. 자막 스킬은 첫 단계(타임라인 매핑)가 실패할 때만 멈추고, 나머지는 이어서 진행한다. 두 스킬은 따로 쓸 수 있다. 프리미어에서 손으로 편집한 타임라인에도 자막 스킬만 쓰면 된다.
...
3 skill(s) installed to /Users/<사용자이름>/.claude/skills
```

`~/.claude/skills/` 에 파일을 복사하지 않고 **바로가기(심볼릭 링크)** 를 만든다. 그래서 스킬의 실제 파일은 계속 저장소의 `skills/` 폴더에 있다.

설치한 뒤 **클로드 코드를 완전히 종료했다가 다시 연다.** `/skills` 를 입력해 목록에 세 스킬이 보이면 된다.

#### 설치가 안 될 때

| 출력 | 원인과 해결 |
|---|---|
| `! 프리미어-컷편집 already exists as a real directory` | `~/.claude/skills/` 에 같은 이름의 폴더가 이미 있다(예전에 복사해 둔 것 등). 그 폴더를 다른 곳으로 옮긴 뒤 다시 실행한다. 설치 스크립트는 직접 만든 폴더일 수 있어서 지우지 않는다 |
| 설치됐는데 클로드 코드 목록에 없다 | 클로드 코드를 완전히 다시 시작했는지 확인한다 |
| 링크를 따라가지 못하는 클라이언트 | `npm run skills:install -- --copy` 로 복사해서 설치한다. 이 경우 저장소를 업데이트할 때마다 이 명령을 다시 실행해야 한다 |
...
링크로 설치했으므로 `git pull` 만으로 스킬이 바뀐다. `npm run build` 는 서버 코드가 바뀌었을 때를 위한 것이다. 빌드한 뒤에는 프리미어 브릿지 패널을 오른쪽 클릭 → **Reload** 하고, AI 클라이언트를 다시 시작한다.

스킬 내용을 고치고 싶으면 저장소의 `skills/<스킬 이름>/SKILL.md` 를 고친다. `~/.claude/skills/` 쪽을 열어도 같은 파일이다.

#### 클라이언트별 사용법

스킬 파일은 클로드 코드가 읽는다. 다른 클라이언트에서는 서버에 들어 있는 **MCP 프롬프트**로 같은 절차를 실행한다.

| 클라이언트 | 컷편집 | 자막 |
|---|---|---|
| **클로드 코드** | 「컷편집 시작하자」 | 「자막 검수 시작하자」 |
| **클로드 데스크톱, Codex 등** | 프롬프트 목록에서 `cut_edit_workflow` | 프롬프트 목록에서 `caption_review_workflow` |

MCP 프롬프트에는 절차가 들어 있고, 스킬에는 그 절차에 더해 기준값과 확인 단계가 들어 있다. 클로드 코드를 쓸 수 있다면 스킬을 쓰는 편이 안전하다.
